  ...
  $.each($(form).serializeArray(), function () {
    if (result[this.name]) {
      ...
      if ($.isArray(result[<string>this.name])) {
        ...
        result[this.name] = $.merge(
          Array(result[this.name]),
          Array(this.value),
        );
      }
    } else {
...
export function initializeFormSubmits() {
  ...
  $("form#login").on("submit", function (e) {
    ...
    tryCatchPopup(async () => {
      ...
      return afterLogin(
        { admin: response["admin"] || false, teacher: response["teacher"] } ||
          false,
      );
    });
  });
...
export function changeUserEmail(username: string, email: string) {
  modal.prompt(
    "Please enter the corrected email",
    email,
    async function (correctedEmail) {
      ...
      try {
        ...
        modal.notifyError(
          ["Error when changing the email for user", username].join(" "),
        );
      }
    },
  );
