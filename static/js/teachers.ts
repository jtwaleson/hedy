...
import {
  ...
  clearUnsavedChanges,
  hasUnsavedChanges,
} from "./browser-helpers/unsaved-changes";
      ...
      .done(function (response) {
        if (response.achievement) {
          showAchievements(
            ...
            false,
            "/for-teachers/customize-class/" + response.id,
          );
        } else {
...
export function join_class(id: string, name: string) {
  $.ajax({
    ...
    .fail(function (err) {
      if (err.status == 403) {
        //The user is not logged in -> ask if they want to
...
export function remove_student(
  class_id: string,
  student_id: string,
  prompt: string,
) {
...
export function create_adventure(prompt: string) {
  modal.prompt(prompt, "", function (adventure_name) {
    $.ajax({
      ...
      .done(function (response) {
        window.location.pathname =
          "/for-teachers/customize-adventure/" + response.id;
      })
      .fail(function (err) {
        return modal.notifyError(err.responseText);
      });
...
function update_db_adventure(adventure_id: string) {
  ...
  const content = DOMPurify.sanitize(
    <string>$("#custom_adventure_content").val(),
  );
...
export function update_adventure(
  adventure_id: string,
  first_edit: boolean,
  prompt: string,
) {
...
function show_preview(content: string) {
  ...
  if (typeof level !== "string") {
    throw new Error(`Expected level to be string, got '${name}'`);
  }

  let container = $("<div>");
  container.addClass(
    "preview border border-black px-8 py-4 text-left rounded-lg bg-gray-200 text-black",
  );
...
export function change_password_student(
  username: string,
...
export function create_accounts(prompt: string) {
  modal.confirm(prompt, function () {
    ...
    $(".account_row").each(function () {
      if ($(this).is(":visible")) {
        //We want to skip the hidden first "copy" row
    ...
    $.ajax({
      ...
      .done(function (response) {
        if (response.error) {
          ...
          modal.notifySuccess(response.success);
          if (
            $("input[name='download_credentials_checkbox']:checked").val() ==
            "yes"
          ) {
...
export function initializeCustomizeClassPage(
  options: InitializeCustomizeClassPageOptions,
) {
...
export function initializeClassOverviewPage(
  _options: InitializeClassOverviewPageOptions,
) {
